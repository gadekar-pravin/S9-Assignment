...
The agent's power comes from its ability to use specialized tools. These tools are organized into three servers:
...
#### Step 1: User Input (agent.py)
```
User types question → agent.py receives it → Creates AgentContext
```
...
Sends to AI model:
  - User question
  - Available tool servers (math, documents, websearch)
  ↓
...
- How to plan in conservative mode
- One tool at a time
...
**Target Audience**: Non-technical stakeholders, business analysts, project managers
